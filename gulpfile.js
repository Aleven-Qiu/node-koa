...
gulp.task("concat:js", function() {
    return gulp.src([
            ...
            buildUrl + '/assets/js/Page.js',
            buildUrl + '/assets/js/vue-components.js',
            buildUrl + '/assets/js/vue-filters.js',
        ])
        .pipe(concat('main.js'))
        .pipe(gulp.dest(buildUrl + '/assets/js'));
})
...
gulp.task('replace:config', function() {
    return gulp.src([buildUrl + '/config.js'])
        .pipe(replace('http://127.0.0.1:8085', 'http://qnpm-zane.seosiwei.com'))
        .pipe(replace('localhost', '10.13.16.116'))
        .pipe(replace(/USER(.+)?root'/, "USER:'xjapc'"))
        .pipe(replace(/PASSWORD(.+)?123456'/, "PASSWORD:'1vzFPu83xPbv'"))
        .pipe(replace('use(KoaLogger())', ""))
        .pipe(replace('/Users/MacBook/Desktop/work/qmx-npm-ssr-node/', "/data/qmx-npm-ssr-node/"))
        .pipe(replace('/Users/MacBook/Desktop/zane/storage/', "/data/sinopia/storage/"))
        .pipe(replace('/Users/MacBook/Desktop/zane/components/', "/data/qmx-qnpm-modules/"))
        .pipe(gulp.dest(buildUrl));
});
...
gulp.task('build', gulpSequence(
    ...
    ['babel:server:gen','babel:server:con','babel:server:routers','babel:server:tool'],'vue:back'
));
